...
func (c *Client) UpdateAllGroupMappings(auth_server_id int, mapping []GroupMapping) ([]GroupMapping, error) {
	// Overwrites the group mapping with the given mapping list

	var resp []GroupMapping
	err := c.makeRequest(http.MethodGet, fmt.Sprintf("/v2/prototype/authenticationserver/%d/update_group_mapping", auth_server_id), mapping, &resp)
	return resp, err
}
